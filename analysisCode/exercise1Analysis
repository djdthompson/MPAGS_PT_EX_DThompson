...
def plotFits(n,momentum,p=None):
    ...
    if mode=="Circle":
        ...
        plt.gca().add_patch(pat.Rectangle((-1,-1),2,2,ec='g',label="B-Field"))
...
xmin, xmax = initMom-initMom/100*3,initMom+initMom/100*3
...
normaliser=len(momentumValues)*np.abs(h[1][1]-h[1][0])
